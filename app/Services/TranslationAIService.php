...
final class TranslationAIService
{
    ...
    public function translate(array $content, string $context = ''): array
    {
        ...
        // Build prompt dynamically based on content fields
        $hasFullDescription = isset($content['full_description']) && $content['full_description'];

        $fullDescSection = $hasFullDescription
            ? "\n## Full Description:\n{$content['full_description']}"
            : '';

        $responseFields = $hasFullDescription
            ? 'name, description, full_description, and includes'
...
## Description:
{$content['description']}{$fullDescSection}{$includesText}

Respond with a JSON object where each key is a locale code, containing: {$responseFields} (array of translated include texts in the same order).
PROMPT;

        $schema = $this->buildJsonSchema(
            count($content['includes'] ?? []),
            $hasFullDescription
        );
...
PROMPT;
    ...
    private function buildJsonSchema(int $includesCount, bool $hasFullDescription = true): array
    {
        $properties = [
            'name' => ['type' => 'string'],
        ...
        if ($hasFullDescription) {
            $properties['full_description'] = ['type' => 'string'];
